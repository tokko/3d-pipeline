...
// Default settings — port overridable via env var UNREAL_MCP_PORT
...
UUnrealMCPBridge::UUnrealMCPBridge()
{
    ...
    AssetPipelineCommands = MakeShared<FUnrealMCPAssetPipelineCommands>();
}
...
void UUnrealMCPBridge::Initialize(FSubsystemCollectionBase& Collection)
{
    ...
    if (!PortEnv.IsEmpty())
    {
        ...
        if (Parsed > 0 && Parsed <= 65535)
        {
            ...
            UE_LOG(LogTemp, Display, TEXT("UnrealMCPBridge: Using port %d from UNREAL_MCP_PORT"), Port);
        }
        else
        {
            UE_LOG(LogTemp, Warning, TEXT("UnrealMCPBridge: UNREAL_MCP_PORT='%s' invalid, falling back to %d"), *PortEnv, Port);
        }
    }
    ...
    AsyncTask(ENamedThreads::GameThread, [this, CommandType, Params, Promise = MoveTemp(Promise)]() mutable
    {
        ...
        try
        {
            ...
            // Editor Commands (including actor manipulation)
                     ...
                     CommandType == TEXT("set_actor_static_mesh"))
            {
            ...
            // Asset Pipeline Commands (3d-pipeline fork) — Python-backed
            else if (FUnrealMCPAssetPipelineCommands::OwnsCommand(CommandType))
            {
                ResultJson = AssetPipelineCommands->HandleCommand(CommandType, Params);
            }
            else
            {
